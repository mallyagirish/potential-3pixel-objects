...
# grid => a numpy 2D array
# val_of_interest => scalar integer
def get_potential_swaps(grid, val_of_interest):
    nRows = grid.shape[0]
    nCols = grid.shape[1]
    ...
    # Return the candidates as a list of tuples (each contains two tuples - source and dest coordinates)
    return [(source, dest) for source, dest in zip(candidate_source_coords, candidate_destination_coords)]   
